...
const About = ({ setView, isSummary = false, theme, isDarkMode }) => {
  return (
    <div className={!isSummary ? "pt-32 pb-20" : ""}>
      <section
        className={`py-24 px-6 border-y ${theme.cardBorder} ${theme.bg}`}
      >
                ...
                >
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 text-sm">
                      <div
                        className={`p-2 rounded-lg ${theme.bg} ${theme.textMuted}`}
                      >
                        <MapPin size={16} />
                      </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-neutral-200 dark:border-neutral-800 flex justify-center gap-4">
                    <MagneticButton
                      ...
                      className={`p-3 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors ${theme.textMuted} hover:${theme.accent}`}
                    >
                      <Github size={20} />
                    </MagneticButton>
              ...
              <div className="md:col-span-7 lg:col-span-8 space-y-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-8">
                  {[
                    ...
                    <div
                      key={i}
                      className={`p-4 rounded-2xl border ${theme.cardBorder} ${theme.bg} flex flex-col`}
                    >
      ...
      {!isSummary && (
        <section className="py-20 px-6">
          <div className="max-w-4xl mx-auto">
            <Reveal>
              <h3 className={`text-3xl font-bold mb-12 ${theme.text}`}>
                Experience
              </h3>
              <div
                className={`space-y-12 border-l-2 ${theme.cardBorder} pl-8 ml-4`}
              >
                {EXPERIENCE_DATA.map((job, i) => (
                  <div key={i} className="relative">
                    <div
                      className={`absolute -left-[41px] top-0 p-2 rounded-full border-4 ${
                        isDarkMode ? "border-neutral-900" : "border-[#FDFBF7]" // Fixed border for light mode
                    ...
                    <div
                      className={`flex items-center gap-2 mb-2 ${theme.textMuted}`}
                    >
                      <span className="font-medium">{job.company}</span>
                      <span>•</span>
                      <span className="text-sm">{job.period}</span>
                    </div>
                    <p className={theme.textMuted}>{job.desc}</p>
                  </div>
                ))}
              </div>
            </Reveal>
